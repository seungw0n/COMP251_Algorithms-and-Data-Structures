...
public class Chaining {
    ...
    public int chain (int key) {
     ...
     int result = ((this.A * key) % power2(this.w)) >> (this.w - this.r );
        return result;
    }
    ...
    public int insertKey(int key){
     ...
     // If that slot is empty, insert the key directly
     if(this.Table.get(hashValue).size() == 0) {
      numCollision = 0;
      this.Table.get(hashValue).add(key);
     } else { //Otherwise, count the number of collisions, and insert the key.
      numCollision = this.Table.get(hashValue).size();
      this.Table.get(hashValue).add(key);
     }
